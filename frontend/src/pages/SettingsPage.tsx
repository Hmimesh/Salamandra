...
export function SettingsPage() {
  ...
  async function importInventory(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      notify("CSV must be UTF-8 encoded. Export as CSV UTF-8 and try again.");
      event.target.value = "";
      return;
    }
    try {
  ...
  return (
    <div className="page settings-page">
      ...
      <div className="settings-grid">
        {state!.auth.learning_summary_allowed ? <LearningSummary /> : null}
        ...
        <section className="settings-section appearance-section"><header><span><Sun size={20} /></span><div><h2>Appearance</h2><p>Make the workspace comfortable for your shift and screen.</p></div></header><div className="preference-list"><div className="preference-row"><span><strong>Theme</strong><small>Follow your device or choose a fixed mode.</small></span><div className="segmented-control" aria-label="Theme">{(["system", "light", "dark"] as const).map((theme) => { const Icon = theme === "system" ? Monitor : theme === "light" ? Sun : Moon; return <button key={theme} type="button" className={preferences.theme === theme ? "active" : ""} aria-pressed={preferences.theme === theme} onClick={() => void savePreferences({ theme })}><Icon size={15} /><span>{theme}</span></button>; })}</div></div><div className="preference-row"><span><strong>Text size</strong><small>Changes headings, forms, tables, dialogs, and navigation.</small></span><select value={preferences.font_scale} onChange={(event) => void savePreferences({ font_scale: event.target.value as typeof preferences.font_scale })}><option value="compact">Compact</option><option value="comfortable">Default</option><option value="large">Large</option><option value="largest">Largest</option></select></div><div className="preference-row"><span><strong>Information density</strong><small>Choose tighter lists or more breathing room.</small></span><div className="segmented-control compact-segments"><button type="button" className={preferences.density === "compact" ? "active" : ""} onClick={() => void savePreferences({ density: "compact" })}><Rows3 size={15} />Compact</button><button type="button" className={preferences.density === "comfortable" ? "active" : ""} onClick={() => void savePreferences({ density: "comfortable" })}>Comfortable</button></div></div></div></section>
