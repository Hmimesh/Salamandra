  ...
  return (
    <div className="page settings-page">
      ...
      <div className="settings-grid">
        ...
        <section className="settings-section"><header><span><ShieldCheck size={20} /></span><div><h2>Workspace</h2><p>Company access and seat usage.</p></div></header><div className="plan-display"><div><span>{state!.organization.plan}</span><strong>{state!.organization.name}</strong></div><span>{state!.organization.seat_count} of {state!.organization.seat_limit} seats</span></div><div className="seat-track"><span style={{ width: `${Math.min(100, ((state!.organization.seat_count || 0) / (state!.organization.seat_limit || 1)) * 100)}%` }} /></div><div className="settings-facts"><span><strong>Organization ID</strong>{state!.organization.id}</span><span><strong>Warehouse</strong>{state!.organization.warehouse}</span></div><footer><span>Owner and admin roles can add accounts.</span><button className="button button-secondary" onClick={() => navigate("/team")}>Manage team</button></footer></section>
        ...
        <section className="settings-section data-company-section"><header><span><Database size={20} /></span><div><h2>Data & company</h2><p>Local storage, support, and policy.</p></div></header><button className="settings-nav-row" onClick={() => navigate("/contact")}><span><strong>Contact support</strong><small>Operations and account assistance</small></span><ChevronRight size={18} /></button><button className="settings-nav-row" onClick={() => navigate("/legal")}><span><strong>Legal & privacy</strong><small>Terms and local data handling</small></span><ChevronRight size={18} /></button><div className="storage-path"><strong>Local data path</strong><code>{state!.save_path}</code></div></section>
      </div>
    </div>
  );
}
